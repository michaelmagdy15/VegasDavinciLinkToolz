...
print("All video clips between 38.0s and 52.0s across ALL tracks:")
for ti, t in enumerate(tl.get('tracks', [])):
    ...
    for ci, c in enumerate(t.get('clips', [])):
        ...
        if (st <= 52000 and end >= 38000):
            fn = str(c.get('media_path')).encode('ascii', errors='replace').decode()
            cname = str(c.get('name')).encode('ascii', errors='replace').decode()
            print(f"Track {ti} ('{tname}'): st={st:.1f}ms ({st/1000:.2f}s) to end={end:.1f}ms ({end/1000:.2f}s) | clip='{cname}' | file='{fn}'")
