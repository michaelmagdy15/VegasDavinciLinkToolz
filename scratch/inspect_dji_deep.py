...
print(f"Found {len(found_paths)} DJI original/proxy files.")

for clip, path in list(found_paths.items())[:5]:
    print(f"\n--- Checking {clip} ---")
    print("Path:", path)
    # Check binary for D-Log / D-Cinelike strings in first 2MB
    with open(path, 'rb') as f:
        head = f.read(2 * 1024 * 1024)
        for term in [b'D-Log', b'dlog', b'D-Cinelike', b'cinelike', b'Normal', b'HLG', b'color_mode']:
            if term.lower() in head.lower():
                print(f"  Found term in header: {term}")
                
    # Also run ffprobe for all metadata streams
    cmd = [
